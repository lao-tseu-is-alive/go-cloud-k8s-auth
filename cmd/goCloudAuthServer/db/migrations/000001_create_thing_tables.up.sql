...
CREATE SCHEMA go_auth_db_schema;
CREATE TYPE go_cloud_auth_status_type AS ENUM (
    'Planifié', -- This state refers to somego_cloud_auth that is in the early stages of development, where the design and construction plans have been finalized, but physical construction has not yet begun.
    'En Construction', -- Somego_cloud_auth is in the process of being built.
    'Utilisé', -- The go_cloud_auth is now being utilized for its intended purpose.
    'Abandonné', -- The go_cloud_auth is no longer in use, being utilized
    'Démoli'
    );
-- TO LIST THOSE ENUM VALUES:
--SELECT unnest(enum_range(NULL::go_cloud_auth_status_type))
/*
-- Table structure for table go_cloud_auth for project go-cloud-auth
-- version : 0.0.4
*/
CREATE TABLE IF NOT EXISTS go_auth_db_schema.go_cloud_auth
(
    ...
    id                 uuid    not null
        constraint pk_go_cloud_auth primary key default gen_random_uuid(),
    type_id            integer not null,
    name               text    not null constraint go_auth_db_schema_unique_name	unique
    ...
    external_ref       text,
    build_at           timestamp,
    status             go_cloud_auth_status_type,
    contained_by       uuid,
...
SELECT AddGeometryColumn('go_auth_db_schema', 'go_cloud_auth', 'position', 2056, 'POINT', 2);
CREATE INDEX idx_go_cloud_auth_geom_gist ON go_auth_db_schema.go_cloud_auth USING gist (position);
CREATE INDEX idx_go_cloud_auth_type_id ON go_auth_db_schema.go_cloud_auth (type_id);


--
-- Table structure for table `TypegoCloudAuth` generated from model 'TypegoCloudAuth'
--
CREATE TABLE IF NOT EXISTS go_auth_db_schema.type_go_cloud_auth
(
    id                 serial
        constraint pk_type_go_cloud_auth primary key,
    name               text                    not null,
    description        text,
    comment            text,
    external_id        integer,
...
alter table go_auth_db_schema.go_cloud_auth
    add constraint go_cloud_auth_type_go_cloud_auth_id_fk
        foreign key (type_id) references go_auth_db_schema.type_go_cloud_auth;

-- #### BEGIN OF DATA FROM GOELAND
-- imported from TypegoCloudAuth in goeland as of 2023-07-21
INSERT INTO go_auth_db_schema.type_go_cloud_auth (id, name, description, comment, external_id, table_name, inactivated, inactivated_time,
...
UPDATE go_auth_db_schema.type_go_cloud_auth
...
create index type_go_cloud_auth_text_search_index
    on go_auth_db_schema.type_go_cloud_auth using gin (text_search);

SELECT setval('go_auth_db_schema.type_go_cloud_auth_id_seq', max(id)) FROM go_auth_db_schema.type_go_cloud_auth;
